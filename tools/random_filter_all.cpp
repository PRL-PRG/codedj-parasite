...
std::unordered_map<std::unordered_map<uint64_t, std::pair<String, std::unordered_set<uint64_t>>> GetProjectIds() {
    std::cout << "Filtering projects for language " << language << std::endl;
    std::unordered_map<uint64_t, std::unordered_set<uint64_t>> result;
    std::unordered_set<String> languages({
...
"C","C++","C#","Objective-C","Go","Java","CoffeeScript","JavaScript","TypeScript","Ruby","PHP","Python","Perl","Clojure","Erlang","Haskell","Scala"
    ...
    CSVReader::Parse(InputDir + "/projects.csv", [&](std::vector<std::string> & row) {
        if (languages.find(row[5]) == languages.end() || row[8] == "1" || row[7] != "\\N")
            return;
        result[std::stoull(row[0])].first = row[5];
    });
    std::cout << "    " << result.size() << " projects found" << std::endl;
    return result;
}

void AssignCommitsToProjects(std::unordered_map<uint64_t, std::pair<String, std::unordered_set<uint64_t>>> & projects) {
...
void FilterSmallProjects(std::unordered_map<uint64_t, std::pair<String, std::unordered_set<uint64_t>>> & projects, size_t cutoff) {
...
void SampleProjects(std::unordered_map<uint64_t, std::unordered_set<uint64_t>> & projects, size_t num) {
    ...
    std::unordered_map<String, std::unordered_set<uint64_t>> by_language;
...
void FilterProjects(std::unordered_map<uint64_t, std::pair<String, std::unordered_set<uint64_t>>> & projects) {
